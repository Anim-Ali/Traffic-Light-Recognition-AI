...
def load_data(data_dir):
    ...
    all_labels = []
    # For each folder in the given directory
    for x in range(NUM_CATEGORIES):
        # save path of folder in folder_path
        folder_path = os.path.join(data_dir,str(x))
        # for each file in the folder
        for file in os.listdir(folder_path):
            # save path of file in file_path
            file_path = os.path.join(folder_path, file)
            # read image from file_path
            img = cv2.imread(file_path)
            # resize image
            resized_img = cv2.resize(img, dsize=(IMG_WIDTH, IMG_HEIGHT), interpolation=cv2.INTER_CUBIC)
            # add image to data array
            data.append(resized_img)
            # add label to label array
            all_labels.append(x)
        # print("Now reading files from folder No: ", x)

    # convert data array to numpy array
    images = np.array(data)

    #print("Working with {0} images".format(len(images)), "type", type(images), "Labels length:", len(all_labels))
...
def get_model():
    ...
    """
    # Modified code from handwriting.py
    ...
    model = tf.keras.models.Sequential([
        ...
        # 3rd Convolutional layer. Learn 32 filters using a 3x3 kernel
        tf.keras.layers.Conv2D(
            32, (3, 3), activation="relu", input_shape=(30, 30, 3)
        ),
